...
import { bannerImagesDtoSchema, BannerImagesDtoFactory } from '@/features/banners/dtos/banner-image.ts';
...
export const adminBannersGetImages = bannersAdminBase
  .route({
    ...
    description: 'Returns the mobile / tablet / desktop images for a banner',
  })
  ...
  .handler(async ({ input, context: { user }, errors }) => {
    ...
    return BannerImagesDtoFactory.fromImageDtos(images);
  });
