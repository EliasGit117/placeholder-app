...
import { bannerDevicePurpose, bannerDevices } from '@/features/banners/consts/banner-devices.ts';

export const adminBannersDeleteImage = bannersAdminBase
  .route({
    method: 'DELETE',
    path: `${bannersAdminPath}/{bannerId}/images/{device}`,
    summary: 'Delete a banner device image',
    description: 'Removes the mobile / tablet / desktop image (and its storage object) from a banner',
  })
  .errors({ FORBIDDEN: {}, NOT_FOUND: {} })
  .use(authMiddleware)
  .input(z.object({
    bannerId: z.coerce.number().int().positive(),
    device: z.enum(bannerDevices),
  }))
  ...
  .handler(async ({ input, context: { user }, errors }) => {
    ...
    const [image] = await ImageService.findByResource(
      ...
      bannerDevicePurpose[input.device]
    );
